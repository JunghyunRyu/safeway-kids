...
import { listBookings, acceptBooking, type CcBooking } from '../../api/bookings';

export default function BookingRequestScreen({ navigation }: any) {
  const [requests, setRequests] = useState<CcBooking[]>([]);
  const load = async () => { try { setRequests(await listBookings('pending')); } catch {} };
  useEffect(() => { load(); }, []);

  const handleAccept = async (id: string) => {
    Alert.alert('예약 수락', '이 예약을 수락하시겠습니까?', [
      { text: '취소', style: 'cancel' },
      { text: '수락', onPress: async () => { try { await acceptBooking(id); await load(); Alert.alert('수락됨'); } catch {} } },
    ]);
  };
