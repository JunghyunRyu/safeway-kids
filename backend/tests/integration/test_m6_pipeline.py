...
from app.modules.scheduling.models import DailyScheduleInstance, RoutePlan, ScheduleTemplate
...
async def _create_full_scenario(db: AsyncSession, target_date: date) -> dict:
    """Create a complete test scenario: academy, vehicles, drivers, students, templates."""
    # Admin
    admin = User(id=uuid.uuid4(), role=UserRole.PLATFORM_ADMIN, phone="01099990000", name="테스트 관리자")
    db.add(admin)
    await db.flush()
    ...
    # Drivers
    driver1 = User(id=uuid.uuid4(), role=UserRole.DRIVER, phone="01099991111", name="기사1")
    driver2 = User(id=uuid.uuid4(), role=UserRole.DRIVER, phone="01099992222", name="기사2")
    ...
    # Parent + Students
    parent = User(id=uuid.uuid4(), role=UserRole.PARENT, phone="01099993333", name="학부모1")
    db.add(parent)
    await db.flush()

    students = []
    for i in range(3):
        s = Student(guardian_id=parent.id, name=f"학생{i+1}", date_of_birth=date(2018, 1, 1+i))
        db.add(s)
        students.append(s)
    await db.flush()
    ...
    return {
        "admin": admin, "academy": academy, "drivers": [driver1, driver2],
...
class TestDailyPipeline:
    ...
    async def test_pipeline_creates_schedules_and_routes(self, db_session):
        ...
        scenario = await _create_full_scenario(db_session, target)
        ...
        assert result["assignments_created"] == 2  # 2 vehicles → 2 drivers
    ...
    async def test_pipeline_with_no_templates(self, db_session):
        ...
        # Create scenario for Monday (templates for weekday 0)
        await _create_full_scenario(db_session, date(2026, 3, 16))  # Monday

        from app.modules.scheduling.scheduler import run_daily_pipeline
        # Run pipeline for Sunday (weekday 6) — no templates match
        result = await run_daily_pipeline(db_session, date(2026, 3, 15))  # Sunday
...
class TestDriverRouteEndpoint:
    ...
    async def test_driver_gets_route(self, client, db_session, driver_user, driver_token):
        ...
        vehicle = Vehicle(id=uuid.uuid4(), license_plate="88가8888", capacity=15)
        ...
        plan = RoutePlan(
            ...
            stops=[
                {"stop_id": "s1", "order": 1, "latitude": 37.501, "longitude": 127.001, "student_name": "학생1"},
                {"stop_id": "s2", "order": 2, "latitude": 37.502, "longitude": 127.002, "student_name": "학생2"},
            ],
            total_distance_km=1.5, total_duration_min=8.0, generated_by="vrp-tw-v1",
        )
        db_session.add(plan)
        await db_session.flush()
    ...
    async def test_driver_no_assignment_returns_null(self, client, driver_token):
    ...
    async def test_parent_cannot_access_driver_route(self, client, parent_token):
...
class TestPipelineEndpoint:
    ...
    async def test_pipeline_via_api(self, client, db_session, academy_admin_user, academy_admin_token):
